...
export function useAppLock(): AppLockState {
  ...
  useEffect(() => {
    if (status === 'locked') {
      void authenticate(); // biometrics → PIN modal on failure
    }
  }, [status]);
...
- [Open the biometric app-lock spec](./spec-biometric-app-lock.md)
- With an anchor (exact ID): [to section 3](./spec-biometric-app-lock.md#3-技術仕様)
- With an anchor (resolved by heading text): [to the overview](./spec-biometric-app-lock.md#概要)
